...
import com.softtanck.imusic.view.tools.Utils;

public class Slider extends CustomView {

	private int backgroundColor = Color.parseColor("#4CAF50"); // #4CAF50
	private Ball ball;
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN || event.getAction() == MotionEvent.ACTION_MOVE) {
				...
				if ((event.getX() <= getWidth() && event.getX() >= 0)) {
					...
					if (numberIndicator != null) {
						...
						numberIndicator.indicator.finalSize = getHeight() / 2 + 10;
	...
	protected void setAttributes(AttributeSet attrs) {
		...
		RelativeLayout.LayoutParams params = new LayoutParams(Utils.dpToPx(10, getResources()), Utils.dpToPx(10, getResources()));
	...
	class Indicator extends RelativeLayout {
		...
		protected void onDraw(Canvas canvas) {
			...
			if (animate && size >= finalSize)
				animate = false;
			if (animate == false) {
				ViewHelper.setX(numberIndicator.numberIndicator, (ViewHelper.getX(ball) + Utils.getRelativeLeft((View) ball.getParent()) + ball.getWidth() / 2)
						- size);
